...
<body>
	...
	%>
	Book(s) (ISBN =
	<%=request.getAttribute("borrowedISBNs")%>) were borrowed by
	<%=request.getAttribute("borrower")%>.
	<%
		}
	%>
	<%
		if (!(null == request.getAttribute("borrowedISBNs"))
				&& (!(null == request.getAttribute("notBorrowedISBNs")))) {
	%>
	<br />
	<br />
	<%
	...
	%>
	Book(s) (ISBN =
	<%=request.getAttribute("notBorrowedISBNs")%>) were already in the
	possession of
	<%=request.getAttribute("borrower")%>
	and have not been borrowed.
	<%
		}
	%>
	...
	<form method="GET" action='librarydbforgoogleapps' name="borrow">
		<input type="submit" value="Back" name="back"> <input
			type="hidden" name="page" value="borrow" />
	</form>
</body>
</html>